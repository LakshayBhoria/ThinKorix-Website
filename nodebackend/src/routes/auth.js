...
router.post('/forgot-password', async (req, res) => {
  ...
  const user = await auth.getUserByEmail(email);
  const generic = { message: "If an account exists for that email, we've sent a reset link. Check your inbox — and your spam/junk folder, since automated mail sometimes lands there." };
  ...
  try {
    result = await mailer.sendMail({
      ...
      text: `Hi ${user.name},\n\nUse this link to reset your password (valid for 1 hour):\n${resetUrl}\n\nIf you didn't request this, you can ignore this email.\n\n(Don't see this in your inbox? Check your spam/junk folder.)`
    });
  } catch (err) {
    // A real SMTP failure (bad creds, provider rejecting, etc.) must never
    // turn into a 500 for the person resetting their password, and must
    // never leak the reset link either — that would be a way to hijack any
    // account just by knowing its email. Log it so an admin can fix the
    // SMTP_* env vars, and still respond with the same generic message.
    console.error(`[auth] forgot-password: sendMail failed for ${user.email} — ${err.message}`);
