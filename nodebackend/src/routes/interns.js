...
router.post('/:internNo/resend', auth.attachUser, auth.requireRole('admin'), async (req, res) => {
  ...
  let offerPath = path.join(config.PATHS.offersOut, r.offerLetterFile);
  let certPath = path.join(config.PATHS.certsOut, r.certificateFile);
  ...
  // failure is what was surfacing as a 502.
  try {
    if (!fs.existsSync(offerPath) || !fs.existsSync(certPath)) {
